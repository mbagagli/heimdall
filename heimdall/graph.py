...
# ---> @develop --> the following `plot_graph` should have been shifted already

# # ===================================================================
# # ===================================================================  UTILS
# # ===================================================================

# # @develop Move this function to somewhere else (like creating a dedicated module)
# def plot_graph(nodes, edges, clusters=None, fig_title="Graph Network",
#                show=False, store=False, store_name="GNN.pdf"):
#     """ Edges must be in shape [2, nedges] """
...
# Assuming you have your dataset ready as a list of snapshots.
# Each snapshot is a dictionary with node features 'x', edge connections 'edge_index',
# edge features 'edge_attr' (if available), and labels 'y' for each node at time 't'.

data_list = [
    {
        'x': node_features_t,           # Node features at time 't', shape [num_nodes, num_features]
        'edge_index': edge_index,       # Graph connectivity in COO format, shape [2, num_edges]
        'edge_attr': edge_attributes_t, # Edge features at time 't', shape [num_edges, num_edge_features] (if available)
        'y': labels_t                   # Labels for each node at time 't', shape [num_nodes, *]
    }
    for t in range(num_time_steps)
]

# Split the data into train and test sets
train_data, test_data = temporal_signal_split(StaticGraphTemporalSignal(data_list), train_ratio=0.8)

# Create PyTorch DataLoader for training data
train_loader = DataLoader(train_data, batch_size=1, shuffle=True)

# Now, you can loop through the DataLoader in your training function
for time, snapshot in enumerate(train_loader):
    snapshot = snapshot.to(device)  # If you're using a device like 'cuda'
    x = snapshot.x
    edge_index = snapshot.edge_index
    y = snapshot.y

    # Your forward pass would look like this:
    out = model(x, edge_index)

    # Compute the loss here using 'out' and 'y'...

# You would also have a similar loop for testing/validation, just without the gradient updates.
